...
> 当前版本是 Windows x64 内部试用版。使用前须告知候选人 AI 参与和记录方式，并由人工复核；不得用于隐蔽冒充或自动作出录用决定。

- `/`：面试官控制台；
- `/stage`：给 OBS 采集的数字人舞台；
- Windows 内置中文 SAPI 生成本地语音，浏览器 Web Speech 作为兜底，不产生语音合成费用；
- 支持上传自己的 JPEG、PNG、WebP、MP4 或 WebM 数字人素材；
- OpenAI-compatible 文本模型只负责根据候选人回答生成追问；
...
- 数字人舞台：`http://localhost:3000/stage`
- 健康检查：`http://localhost:3000/api/health`

在控制台的“你的数字人画面”中上传生成图片或待机视频。文件保存在本机
`data/avatar`，OBS 舞台会自动切换，无需重启；也可以点击“恢复默认头像”清除素材。

## 候选人语音转写

控制台点击“开始听取候选人”，选择会议窗口或整个屏幕，并勾选共享系统音频。
应用每 10 秒上传一个独立片段，转写文本会追加到候选人回答框，人工确认后再生成追问。
...
1. 安装并启动 OBS Studio。
2. 在“工具 → WebSocket 服务器设置”中启用服务器，默认端口保持 `4455`。
3. 在本项目控制台填写 OBS WebSocket 密码并连接。
4. 点击“一键配置并输出”。项目会创建或更新 `AI Interviewer` 场景和浏览器源，
...
安装虚拟音频驱动。若电脑没有 Node.js 22+ LTS，安装器会在选定档位后通过 winget 的
...
会使用 DPAPI 保护的随机本机密码，控制台自动连接、创建舞台并启动虚拟摄像头，无需再
进入 OBS 菜单复制密码；若 OBS 已由你提前启动，则保留手工连接方式。日志保存在
`.tools/logs`。虚拟音频驱动涉及系统设备和重启，仍需按控制台指引由用户或 IT 明确安装。
启动器会等待 `/api/health` 返回本项目的固定服务标识后才打开浏览器；端口上的其他 HTTP
服务即使返回 200 也不会被误当成本项目。

日常启动使用生产模式，数字人舞台不会出现 Next.js 开发工具图标。修改代码后先运行
`npm run build`；开发调试才使用 `npm run dev`。

控制台和舞台默认只监听 `127.0.0.1`，不会自动向局域网开放。开始面试前必须确认
已向候选人说明 AI 协助、记录保存和人工复核；开场白也会再次说明。历史面试支持
逐条永久删除，删除前会要求确认。
所有修改型 API 还会校验浏览器 `Origin` 和 `Sec-Fetch-Site`，外部网页不能通过普通表单
替换本机数字人素材或触发音频转写；本机命令行安装与检查脚本不受影响。
...
- 当前采用轻量 2D 数字人，不需要独立显卡。
- 候选人语音可从会议窗口或整个屏幕采集和转写，提交追问前仍允许人工校对。
- 当前单人会话会原子保存到 `data/interviews/current.json`，控制台可导出完整 JSON。
- 如果 `current.json` 意外损坏，系统会先保留为 `current.corrupt.*.json`，再恢复最新一份
  结构有效的归档；备份无法创建时会停止写入，避免静默覆盖恢复线索。
- 面试进行中不能直接创建新会话；必须先结束并归档当前面试，防止记录被覆盖。
...
- 可填写岗位要求、面试重点和问题上限；达到上限后自动播放结束语。
- AI 追问不合适时可点击“重生成本题”，原位替换问题，不重复候选人回答或增加题数；
  人工播报仅在面试进行中可用。
- AI 追问只允许围绕岗位能力和明确工作经历；年龄、婚育家庭、宗教政治、民族籍贯、
  健康病史、残障、性别或性取向等个人敏感问题会在本地拦截并重试，仍不合适则改用
  安全的岗位问题。该技术约束不替代企业法律与招聘合规复核。
- 候选人回答以有长度预算的 JSON 数据发送给模型，系统提示明确禁止执行回答中的命令、
