...
## 2026-08-31：通用 OpenAI Realtime 客户端

- 目标：让 OpenAI、Token Plan、千问及其它兼容网关共用 `session.update` → `session.updated` → PCM 的 Realtime 握手，同时保持级联与非 Realtime E2E 路径不变。
- 采用：将供应商无关逻辑收敛到 `openai_realtime.py`，仅按 `baseUrl` 推断 URL/音频方言；Aliyun 使用 `/api-ws/v1/realtime`、`pcm`，空 `voiceId` 时省略 voice 让模型选择默认音色；其它 OpenAI 风格网关保留基址并追加 `/realtime`、使用 `pcm16`、默认 `alloy`。线路显式 `voiceId` 始终优先。真实 Token Plan 握手证明固定 `Cherry` 会被部分 Qwen Realtime 模型以 `Unsupported voice` 拒绝。
- 复用与许可证：复用 OpenAI Realtime 事件协议和项目已有 `websockets` 依赖；`websockets` 为 BSD-3-Clause，维护活跃且现有版本已提供 asyncio 客户端、连接超时和 Windows/Linux wheels。不新增 SDK、依赖、镜像体积、GPU/CPU、内存或付费 API 成本。
- 安全与数据：API Key 仍只作为 WebSocket Authorization header 发往线路配置的服务端；日志仅记录 host、path、model 和限长错误码，不记录 Key、音频或完整查询参数。
- 未采用：OpenAI 或 DashScope 专用 SDK。现有轻量事件适配层已覆盖当前协议，新增 SDK 会造成重叠依赖和更高维护成本。
- 限制：当前使用兼容协议的扁平 `input_audio_format`/`output_audio_format` 字段；若某网关只接受新版嵌套音频结构，需依据真实握手错误单独增加方言，不做静默降级。

## 2026-08-31：Agent 已进房但界面报未接入

- 目标：自动听取连上 LiveKit 后不再误报「语音 Agent 未接入」；进房后 Qwen-Audio Realtime 不再约 10 秒崩溃。
- 采用：无新依赖。`watchAgentForRoom` 不再清掉 connect 期间已到的 `agentPresent`；Qwen 握手按[官方 Realtime WebSocket](https://help.aliyun.com/zh/model-studio/fun-audiochat-realtime-websocket-api) 等 `session.updated` 再推 PCM，`session.update` 去掉 ASR-Realtime 的 `qwen-audio-asr` 字段；失败日志展开 ExceptionGroup 子异常。
- 未采用：加长 8 秒等待、改 LiveKit URL/Token、换 VB-CABLE 或会议声卡。
- 限制：已安装客户端需重新打包后才带 presence 修复；Agent 镜像需重建后 Realtime 才等握手。

## 2026-08-31：自动桥接死房间可点「重新桥接」

- 目标：腾讯会议仍开着时，自动听取会一直 holding 旧 LiveKit 房间；卡片顶栏显示「未启动」、停止键禁用，看起来像卡死。Agent 被重建后旧房间不会再派工。
- 采用：无新依赖。复用 livekit-client 的 `participant.isAgent`；8 秒无 Agent 则文案改为「请点重新桥接」；`requestAutoBridgeRestart` 停掉当前自动会话并建新房间。不中途自动拆房。
- 未采用：会议进程仍在时自动换房间；不为此引入第二套 RTC SDK。
- 限制：当前已安装的客户端需重新打包后才有「重新桥接」按钮；现有版本可取消勾选「自动听取」再勾上，效果相同。

## 2026-08-31：Qwen Realtime 走官方 /api-ws 路径

- 目标：端到端线路的 Token Plan Chat Completions 基址能连上 Qwen-Audio Realtime；此前把 `https://…/compatible-mode/v1` 拼成 `wss://…/compatible-mode/v1/realtime`，握手挂住，无字幕、无 TTS、`AGENT_COMMAND_TIMEOUT`。
- 采用：无新依赖。继续用已有 `websockets`。按[官方 Realtime WebSocket](https://help.aliyun.com/zh/model-studio/fun-audiochat-realtime-websocket-api) 把 `/compatible-mode/v1` 映射为同源 `/api-ws/v1/realtime`；Token Plan 的 WorkspaceId 即为 `token-plan`。`session.update` 音频格式用官方 `pcm`。握手 `open_timeout=10`，日志只记 host/path/model。
- 未采用：阿里云 DashScope Python SDK / OmniRealtimeConversation（体积与 LiveKit Agent 进程重复）；不改 `AGENT_INTERNAL_TOKEN`、RTC URL 或语音线路。
- 限制：OpenAI 风格 `…/v1` 仍拼 `/realtime`；已是 `/api-ws/v1/realtime` 的基址保持不变。容器需能出网访问 `token-plan.cn-beijing.maas.aliyuncs.com:443`。

## 2026-08-31：无域名 LiveKit 走公网 IP 明文 ws

- 目标：Windows 客户端能连上 175 上的 LiveKit；当前 Token 签发成功但 `wss://175.27.132.61`（443）导致 ConnectionError。
- 采用：无新依赖。RTC URL 改为 `ws://175.27.132.61:7880`；compose 挂载现有 `livekit.yaml`（`use_external_ip`），关闭 TURN/443；安全组 TCP 7880/7881、UDP 7882。客户端 `livekit-client` 2.21 默认 `/rtc/v1`，与 SFU 1.9.6 的 `/rtc` 不兼容，Room 固定 `singlePeerConnection: false` 走 v0；CSP `connect-src` 增加 `http:`，否则明文 `/rtc/validate` 会被拦、SDK 无法识别 404 并回退。
- 未采用：Let’s Encrypt / nginx WSS（没有域名，不能给纯 IP 签公共证书）；不升级 LiveKit 镜像（计划保持现有 yaml / 1.9.6）。
- 限制：信令为明文 WebSocket；媒体仍依赖 UDP 7882 可达。

## 2026-08-31：语音页选择入口与凭据状态分离

- 目标：会议走哪条链路只在「语音线路管理」启用一条；摘要区不再被读成第三条线路（阿里云 vs 级联 vs 端到端）；侧栏「当前线路」不再和语音线路撞名。
- 采用：零新增依赖。`VoiceRoutesPanel` 增加当前启用横幅；`/settings/speech` 摘要改为「语音凭据连通状态」只展示 Key 连通；`/sessions` 与概览改称「在线会话」。不改 Agent、`voice_routes` API 或热切规则。
- 未采用：在凭据摘要里加线路下拉；把阿里云做成与级联/端到端并列的第三种 mode。
- 限制：新房间仍只读一次启用线路快照；进行中的会话不热切。

## 模型启用、端到端线路下拉与设置库报错修复

- 目标：管理端能启用 Token Plan 官方名单中尚未入库的模型；Realtime 线路下拉能选出已启用的端到端模型；设置库失败时能从日志看到真实 SQL 原因。
- 采用：继续用现有 control-api + PostgreSQL `discovered_models` / `token_plan_*` / `voice_routes`。启用官方-only 模型时 upsert 进 `discovered_models`；Realtime/ASR/TTS 不再要求 Chat Completions「本人实测」；00015 补 `CREATE TABLE IF NOT EXISTS discovered_models`。无新依赖。
- 未采用：为 Realtime 另接计费媒体探测；把视频/图片模型写入语音线路。
- 限制：LLM 仍需本人验证后才能用于级联线路；Goose 不会重跑已应用的 00015，已有库靠 upsert 与 wrap 后的日志定位剩余缺列问题。

## LiveKit-only 会议字幕与互动管线（现行）

- 目标：会议实时字幕与 RTC Token **只保留 LiveKit**；管理端配置多 AI 线路 + 模型目录分类，并在 RTC 页绑定 `cascaded` / `e2e`；桌面与 Agent 按管线消费。
- 采用：迁移 `00013_drop_volcengine_rtc` 去掉火山 RTC 列；`00014_ai_providers_multi` 多线路；`00015_model_catalog` 目录与分类；管理端 Combobox + `POST .../speech/preview`；客户端 `GET /client/settings/pipeline`（E2E 未就绪抛 `E2E_NOT_IMPLEMENTED`，不静默降级）。LiveKit Agent 见 `server/livekit-agent/README.md`。
- 未采用：继续双 RTC 切换；在客户端再维护一套主密钥配置作为生产主路径。
- 限制：compose 仍用 `--profile livekit` 启动 SFU/Agent；公网需 UDP/TURN；历史「火山 RTC」条目仅作当时决策留档，不再代表现行行为。
...
- 采用：扩展已有 Go `control-api` + PostgreSQL（现行迁移号 `00015_model_catalog.sql`；rebase 后排在 `00013_drop_volcengine_rtc` / `00014_ai_providers_multi` 之后）；复用现有 `/models` discover 与 HTTP chat 客户端；管理端原生 Combobox；试听复用 `POST .../speech/preview`。**不引入** LangChain / LangGraph / langchaingo / ADK 或自研 Agent 运行时——本轮只需目录 + 分类 + 绑定，编排框架非必要条件；以后若加 Agent，只调现有 REST，不必改表。
...
- 采用：control-api 新增 `aliyun_cosyvoice.go`（`github.com/coder/websocket` FlowingSpeechSynthesizer）；`ai_provider_configs` 迁移增加 `name`/`is_default` 多行（现行 `00014_ai_providers_multi.sql`）；REST `/ai/providers` CRUD + 模型 CRUD；管理端二级菜单按线路展开。
- 限制：试听仍仅阿里云；桌面主路径读管线/默认线路；密钥加密入库。

## 管理端音色试听与 AI 配置二级菜单

- 目标：管理端选阿里云音色时可直接试听；AI 配置页与语音页一致，默认只显示已配置线路摘要，展开后才改密钥/模型；支持阿里云 token-plan OpenAI Compatible 端点发现模型。
- 采用：复用 control-api 已有阿里云 NLS HTTP `/stream/v1/tts` 合成（与连通探针同源），新增 `POST /api/v1/admin/settings/speech/preview` 返回 `audio/wav`；不新引入 TTS SDK。后续已演进为真正的多线路 CRUD（见上条）；早期 UI 折叠方案保留为交互参考。`/ai/discover` 允许携带草稿 `baseUrl`/`apiKey` 以便未保存前拉模型。无新增 npm/Go 依赖。
...
- 目标：腾讯会议音频桥接真正连通后，在模型和助手舞台就绪时自动开始互动；interim 字幕实时显示在对话记录，final 字幕持久化并触发 AI 追问及 TTS 播报。
...
- 安全与边界：AI 告知选项仅控制开场披露文案，不作为功能门禁；桥接 roomId 作为单场幂等键，避免轮询重复创建或结束后在同一会议中再次启动；interim 字幕仅在浏览器内展示，只有 final 字幕写入本地会话。
- 未采用：新状态管理库、额外 WebSocket/SSE、修改腾讯会议设备选择。现有 React 订阅和会话 API 已覆盖需要，第三方会议设备仍由用户手动选择。

## 2026-08-23：实时字幕并入对话时间线

- 目标：移除工作台右侧独立字幕卡片，在中央对话时间线展示会话前、会话中和结束后的临时字幕，并明确显示当前自动启动状态，避免用户把“ASR 已工作”误认为“AI/TTS 已触发”。
- 结论：零新增依赖，复用 React 19 状态/条件列表渲染、现有 `subtitleSink`、会话状态和 `/api/session` → `speakingText` → `/api/tts` 链路；现有 Next.js Client Component 已覆盖浏览器实时状态需求。
- 数据边界：会话开始前字幕只在内存时间线显示，不写正式记录；运行中只展示最新 interim，final 提交期间显示生成状态，成功后由正式记录替换；会话结束后只展示结束时间之后收到的字幕。
- 未采用：第三方时间线组件或状态库（增加体积和维护面但无必要）；修改或重启 LiveKit/ASR 服务（175 日志已确认音轨订阅、阿里云 ASR 连接和字幕发布正常）。

## 2026-08-23：ASR → AI → TTS 端到端结构化诊断日志

- 目标：用同一 traceId 串联桌面捕获首帧、RTC 连接、字幕接收、自动回答门禁、AI 请求、TTS 合成、舞台播放与虚拟声卡选择，并统一落入客户端 `logs/desktop-startup.log`；只记录状态、耗时、长度和原因码，不记录字幕/回答正文、音频、设备 ID 或凭据。
- 采用：零新增依赖。渲染器将白名单诊断元数据发送到同源 `/api/pipeline-log`，服务端二次校验、清除敏感字段并通过现有 stdout 写入桌面启动日志；AI/TTS 服务端节点直接使用同一格式。LiveKit Agent 的既有 final 发布日志补充 `utterance_id` 和 `final`。
- 调研：`electron-log` 5（MIT、Electron 13+）可把渲染器日志转发到主进程，但需要增加其 IPC 初始化及依赖；Pino 10.3.1（MIT、活跃维护）适合高吞吐 JSON 日志，但本项目已有单一 stdout 文件管道；OpenTelemetry JS 2.x（Apache-2.0、活跃维护）提供完整遥测，但 SDK/Exporter、资源开销和外发数据治理明显超出本地故障诊断需要。
- 未采用：记录完整请求/响应或字幕文本（候选人数据泄露风险）；把日志上传第三方；仅依赖 DevTools console（打包客户端无法稳定留存）；为少量本地事件引入重型遥测栈。
- 已定位现象：175 的对应房间音轨约 50 包/秒，Agent 已订阅并连接阿里云 ASR，final 字幕持续发布；客户端没有 `/api/session` answer 或 `/api/tts`，且 UI 为 idle/0 问并显示未确认知情同意，因此断点是客户端会话门禁，不是驱动、LiveKit 或 ASR 线路。

## 2026-08-23：AI 告知非阻断化与无限对话

- 目标：AI 告知选项只决定开场是否声明 AI 参与，不阻断会话；移除问题数量配置和达到上限后自动结束的行为，由用户手动结束互动。
- 采用：零新增依赖，复用现有 React 状态、Zod 会话边界和本地会话存储。`consentConfirmed` 保留为布尔记录；旧会话中的 `maxQuestions` 继续允许解析，但新会话不写入、业务不执行、界面和导出不展示。
- 未采用：新增表单/状态库或数据迁移。现有会话 JSON/SQLite payload 的宽松兼容足以读取历史字段，强制重写历史数据会增加不必要的迁移风险。
- 行为边界：模型、主工作台播报引擎、会议音频桥接及运行中轮次校验仍是技术门禁；仅手动结束会写入 closing 并进入 `finished`。

## 2026-08-23：主工作台唯一 TTS 播放与严格 VB-CABLE 路由

- 根因：AI 已生成 `speakingText`，但 `/api/tts` 从未被调用；播放逻辑只存在于 `/stage`，而桌面壳拒绝 `_blank` 新窗口且当时无 OBS/舞台页面，因此没有消费者。
- 采用：零新增依赖，将会话播报、测试语音、停止控制、设备解析和播放日志集中到主工作台的 feature-first TTS 控制器；`/stage` 只轮询共享状态用于头像动画和字幕，避免 OBS Browser Source、手动舞台与工作台重复合成播放。
- 输出策略：只允许已验证且按标签重新解析的 VB-CABLE 播放端；线路缺失、`setSinkId` 不支持或选择失败时阻止 `audio.play()` 并显示“AI 声音未送入会议麦克风”。不回退默认扬声器或 Web Speech，避免本机有声但对面无声及回声。
- 状态兼容：保留 `/api/stage-status` 和 `connected` 字段，由主工作台播放控制器心跳维护，界面语义改为“播报引擎在线”；管理端、175、ASR 和 AI 服务不变。

## 2026-08-23：AI TTS 本机监听双路播报

- 目标：同一份 AI 合成音频送入 VB-CABLE 的同时，可选地由本机默认扬声器播放，让操作者确认播报内容、音色和语气；本机监听默认开启并持久化。
- 采用：零新增依赖，复用浏览器标准 `HTMLAudioElement`、现有唯一 TTS 控制器和 localStorage 订阅模式。同一个 Blob URL 创建虚拟线路和本机监听两个播放器，只发起一次 `/api/tts` 请求；本机播放器不设置 `sinkId` 和应用内音量，跟随 Windows 默认输出及系统音量。
- 线路边界：VB-CABLE 是主线路，必须先成功解析、选择并开始播放后才启动本机监听；虚拟线路失败时两路都不播。本机监听关闭或播放失败只记录 `mode=local-monitor` 日志，不中断 `mode=virtual` 对外播报。
- 生命周期：人工介入、紧急静音、新一轮播报和页面卸载会同时停止两路音频，并只释放一次共享 Blob URL。舞台和 OBS 继续保持纯视觉，不参与合成或播放。
- 未采用：第三方音频混音/路由库（双 `Audio` 实例已覆盖需求，新增依赖会扩大安装体积和设备兼容风险）；Web Audio 固定增益（用户要求音量完全跟随 Windows，且无应用内音量滑杆）。

## 2026-08-23：AI 参考模式与点击式人工说话

- 目标：暂停 AI 自动模式时不再显示“字幕未提交”告警；新增可持久化的 AI 参考模式，继续生成并显示回复但不调用 TTS；人工说话由按住改为点击启用/关闭。
- 官方能力调查：React 19 官方 `useState`/`useEffect` 已覆盖开关状态及浏览器事件订阅；浏览器标准 `localStorage` 可保存偏好，`HTMLMediaElement.pause()` 可在切换参考模式或人工介入时停止当前播报。现有 Next.js、Windows、OBS 与浏览器线路均无需改变。
- 采用：零新增依赖，复用现有会话 `speakingText`、工作台 TTS 控制器、人工介入状态机和 localStorage 订阅模式。参考模式只抑制会话回复的 `/api/tts`，字幕/会话记录与测试语音保持原链路。
- 资源、安全与维护：不增加安装体积、GPU/CPU、内存或付费 API 成本；参考模式反而避免 TTS 请求。偏好仅保存布尔值，不保存字幕、候选人数据或密钥。
- 未采用：新状态库、UI 组件库或音频库（现有能力完整且更小）；重新实现音频路由或摄像头驱动（与本次交互无关且风险更高）。

## 2026-08-23：声音刻录上传错误分层与回归保护

- 根因：客户端 `fetchDesktopControlJson` 把 control-api 的未登录、路由缺失、存储未配置、COS 上传/预签名失败和网络异常都折叠为 `null`，最终统一误报为“未配置对象存储”；同时 voice-samples handler/service 未进入上一次 Git 提交，干净 `HEAD` 无法编译 control-api。
- 采用：零新增依赖。新增安全的 control-api 结果类型，只保留 HTTP 状态、限长 `code`/`message`，声音刻录界面按认证、版本、存储、上传、预签名和网络问题分别提示；日志仅记录 operation、status 和 code。
- 服务端：复用现有腾讯云 COS SDK和对象存储配置，把配置读取、PutObject、PresignGet、DeleteObject 分成稳定错误码；预签名失败继续尽力删除已上传样本。
- 安全：不记录 Cookie、音频、COS 凭据、内部错误正文或预签名 URL；上游返回的额外字段不会进入客户端失败对象。
- 验证：Node 行为测试覆盖安全响应解析和中文映射；Go service/handler 测试覆盖 WAV 校验、大小限制、上传、预签名失败清理、删除、401/422/503/500/201/204。
# 2026-08-23 四角色会话与可配置角色话术

- 目标：为桌面会话增加 HR、会议助手、面试官、应聘者四种锁定角色，并由管理端配置角色话术。
- 调研结论：现有 React、Zod、PostgreSQL migration 和 OpenAI-compatible Chat Completions 已覆盖 UI、校验、持久化和 system 指令层级；无需引入第三方提示词或状态管理依赖。
- 官方能力：沿用 OpenAI-compatible `system` 消息承载固定安全规则和角色契约，把对话、资料及管理配置作为低信任数据；管理配置不能覆盖固定规则。
- 许可证与维护：未新增依赖，因此没有新增许可证、安装体积、GPU/CPU、数据去向或付费 API 成本。继续使用项目现有依赖版本和服务端密钥边界。
- 兼容性：新会话必须显式选择角色；旧会话缺字段时读取为面试官。会话保存角色配置快照，管理端修改只影响后续会话。
- 安全：模板只接受 `{{target}}`、`{{topic}}`；固定防提示注入、敏感信息和输出格式规则由代码追加。应聘者资料不足时不得虚构公司、项目或数字。

## 2026-08-23：单用户一次性复刻音色分配

- 目标：每个桌面账号只允许成功分配一个复刻音色，重复点击、双窗口和旧客户端覆盖请求都不能替换已有绑定或继续消耗云端音色位置。
- 官方调查：当前智能语音交互 `CosyVoiceClone` 每次生成带随机后缀的新 `VoiceName`，同一套接口只提供复刻与列表查询，暂不支持更新或删除已复刻音色；每个阿里云 UID 最多复刻 1000 个音色。
- 采用：零新增依赖，复用 PostgreSQL 唯一用户行、现有控制 API 和阿里云 POP 客户端。服务端以 `unallocated`、`allocating`、`allocated` 状态及一次性随机令牌建立原子门禁，成功后永久锁定；明确失败释放，网络超时等不确定结果保持处理中。
- UI：已有音色只展示一次性分配说明和试听；录音、提交和手工音色 ID 输入不再展示。会议确认与音频桥接从设置页移除，手动桥接改由主工作台承载。
- 安全与成本：不新增密钥、音频或完整音色 ID 日志，不向刻录页面返回完整音色 ID；门禁发生在调用云端之前，避免并发产生额外计费或配额占用。

## 2026-08-28：飞书进程切换后的音频桥接自动恢复

- 根因：自动桥接绑定带窗口标题的单个 `feishu.exe` PID；该 PID 退出时客户端主动离开 LiveKit 房间，但自动状态机仍短暂保留旧捕获状态，未立即探测飞书的新窗口 PID。
- 采用：零新增依赖，复用现有自动桥接状态机、5 秒进程探测、LiveKit Client SDK 连接事件与结构化诊断管道。进程退出会原子清除匹配的旧 PID、立即重扫并恢复；旧房间的迟到事件不能停止替代房间。
- 连接状态：LiveKit `Reconnecting`、`Reconnected`、`Disconnected` 向桥接控制器传播；界面不再把断开的旧房间显示为“已自动捕获”，不可恢复的断线进入同一受控重建流程。
- 安全与成本：日志仅记录事件来源、原因码、PID、房间号、provider 和状态，不记录字幕正文、音频、设备 ID、Token 或密钥；不增加安装体积、GPU/CPU、内存及 API 成本。
- 未采用：新增进程监控、状态管理或重连依赖。Electron、Windows 进程枚举和 LiveKit SDK 已提供完整事件面，额外依赖只会扩大维护与供应链风险。

## 2026-08-31：Token Plan 个人版官方模型目录

- 权威来源：阿里云 Model Studio 的 Token Plan 个人版“支持的模型”公开页面；官方目录同步不携带 Token Plan Key。
- 采用：复用 Go 标准库 HTTP、HTML 实体解码、正则和现有 PostgreSQL/goose 能力，事务保存官方目录、页面更新时间、内容 SHA-256 与最近成功时间。解析失败、空名单或页面结构变化不会覆盖上一版成功目录。
- 未引入依赖：不新增 HTML 解析、定时任务或前端状态库，因官方表格模型 ID 结构简单，现有标准库和 24 小时 ticker 足够；因此无新增许可证、安装体积、GPU/CPU、内存或供应链成本。
- 协议边界：文本模型通过 Chat Completions 做本人点击验证；ASR、TTS、Realtime、图片和视频被标注为各自专用协议，管理端不会错误地按 Chat Completions 调用，也不会在验证时隐式产生媒体生成费用。
- 安全：Key 仍只在服务端加密存储；公开目录同步不读取 Key，`GET /models` 只记录 Key 已发现状态，实际模型调用只由本人点击触发，响应和日志不包含 Key。

## 2026-08-31：互动管线与 Windows 客户端语音线路解耦

- 目标：RTC 页只管理 LiveKit Agent 互动管线，语音页独立管理 Windows 客户端线路；厂商凭据卡片编辑时不再锁住线路选择。
- 采用：零新增依赖，复用现有 React 状态、Next.js 表单、`GET/PUT /api/v1/admin/settings/speech` 和 `activeProvider` 字段。客户端线路使用独立脏状态与保存请求，避免覆盖同时编辑的凭据草稿。
- 官方与现有能力：React `useState` 已覆盖草稿、已保存基线与提交状态；原生 `disabled` 属性可阻止未变更或重复提交，不需要表单库或状态库。
- 兼容与安全：数据库和 API 不变，Windows 客户端继续从既有接口读取线路；不新增密钥、日志、安装体积、运行成本或外部数据流。
- 未采用：RTC 保存后推导并同步客户端线路（重新引入职责耦合）；选择即保存（误触会直接影响后续会话）；新增 React 表单或状态管理依赖（现有能力足够）。

## 2026-08-31：统一语音线路与 LiveKit Agent 执行端

- 目标：把 RTC 内嵌管线、旧 pipeline 配置和客户端线路收敛为可命名、复制、测试及原子启用的语音线路；新房间只读取一次启用线路快照。
- 官方方案：继续复用 Apache-2.0 的 LiveKit Agents 与现有 `livekit-client` 房间媒体能力。Agents 提供服务端 STT/LLM/TTS/Realtime 抽象及房间音频发布边界，适合让 Windows 客户端退出模型与密钥执行链路；无需开发音频驱动。
- 阿里云协议：OpenAI-compatible 模型沿用项目既有兼容端点；Qwen Audio Realtime 使用阿里云官方 WebSocket 事件协议和个人版北京端点，不把 Realtime 错当 Chat Completions。现有 Python `websockets` 依赖与适配层可复用，无需新增 SDK。
