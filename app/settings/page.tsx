...
type Diagnostics = { server: boolean; modelConfigured: boolean; stageConnected: boolean; ttsSupported: boolean; voiceCount: number; sapiConfigured: boolean; sapiVoiceCount: number; ttsState: "idle" | "speaking" | "ready" | "error"; ttsError: string; lastSpeechAt: number; mediaReady: boolean; transcriptionConfigured: boolean; transcriptionReady: boolean; transcriptionSource: "aliyun" | "volcengine" | "management" | "environment" | "whisper-cpp" | "none" };
...
export default function SettingsPage() {
  ...
  return <main className="console settingsPage">
    ...
    <section className="settingsGrid">
      ...
      {virtualMode && <SettingSection number="06" title="摄像头与音频" detail="一键授权并检查会议软件最终读取到的画面和声音。"><div className="settingsPair"><VirtualCameraPreview active={virtualCameraActive} onVerifiedChange={handleCameraVerified} /><AudioRouteControl onReadyChange={handleVirtualAudioReady} /></div></SettingSection>}
      {virtualMode && <SettingSection number="07" title="会议软件确认" detail="在入会预览中完成最后一跳确认。"><MeetingHandoffControl prerequisitesReady={obsConnected && virtualCameraActive && virtualCameraVerified && virtualAudioReady} onConfirmedChange={handleMeetingPreviewConfirmed} /></SettingSection>}
    </section>
  </main>;
}
...
function transcriptionDetail(diagnostics: Diagnostics) {
  if (diagnostics.transcriptionReady) {
    ...
    return diagnostics.transcriptionSource === "management" ? "已由管理端配置" : "本机回退服务可用";
  }
  if (diagnostics.transcriptionConfigured) return "管理端转写暂不可用";
  return "请在管理后台配置豆包语音或转写服务";
}
