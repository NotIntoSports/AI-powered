...
export default function SettingsPage() {
  ...
  async function uploadAvatar(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const form = event.currentTarget; const file = (form.elements.namedItem("avatar") as HTMLInputElement).files?.[0]; if (!file) return;
    setUploading(true); setError(""); try { const body = new FormData(); body.append("avatar", file); const response = await fetch("/api/avatar", { method: "POST", body }); const data = await response.json(); if (!response.ok) throw new Error(data.message || "上传失败"); setAvatar(data); form.reset(); } catch (cause) { setError(cause instanceof Error ? cause.message : "上传失败"); } finally { setUploading(false); }
  }
  ...
  return <main className="console settingsPage">
    ...
    <section className="settingsGrid">
      <SettingSection number="01" title="AI 模型" detail="配置数字人自动回应与对话纪要服务。"><ModelSettings /></SettingSection>
      <SettingSection number="02" title="实时字幕" detail="配置火山 RTC AppID、鉴权和字幕语言。"><RtcSettings /></SettingSection>
      <SettingSection number="03" title="数字人形象" detail="选择互动中展示的图片或待机视频。"><article className="card mediaSetup"><div className="cardHeading"><h2>当前画面</h2><span>{avatar.available ? "已启用" : "默认形象"}</span></div><div className="mediaPreview">{avatar.available && avatar.kind === "video" && <video src={`/api/avatar/media?v=${avatar.version}`} autoPlay loop muted playsInline />}{avatar.available && avatar.kind === "image" && <img src={`/api/avatar/media?v=${avatar.version}`} alt="当前数字人素材" />}{!avatar.available && <span>当前使用默认数字人</span>}</div><form onSubmit={uploadAvatar}><input name="avatar" type="file" accept="image/jpeg,image/png,image/webp,video/mp4,video/webm" /><button disabled={uploading}>{uploading ? "正在上传…" : "上传并应用"}</button></form>{avatar.available && <><p className="fileMeta">{avatar.originalName} · {Math.max(1, Math.round((avatar.size || 0) / 1024))}KB</p><button className="secondary" disabled={uploading} onClick={clearAvatar}>恢复默认形象</button></>}</article></SettingSection>
      <SettingSection number="04" title="OBS 与舞台" detail="连接 OBS、配置舞台源并启动虚拟摄像头。"><ObsControl onStatusChange={handleObsStatus} /></SettingSection>
      <SettingSection number="05" title="摄像头与音频" detail="检查会议软件最终读取到的画面和声音。"><div className="settingsPair"><VirtualCameraPreview active={virtualCameraActive} onVerifiedChange={handleCameraVerified} /><AudioRouteControl onReadyChange={handleVirtualAudioReady} /></div></SettingSection>
