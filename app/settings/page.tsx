...
export default function SettingsPage() {
  ...
  const handleObsStatus = useCallback((status: {
    ...
    setReadinessVerification("obsConnected", status.connected);
    setReadinessVerification("virtualCameraActive", status.virtualCameraActive);
    if (!status.virtualCameraActive) setVirtualCameraVerified(false);
  }, []);

  const handleCameraVerified = useCallback((ready: boolean) => {
    if (!snapshotLoadedRef.current) return;
    setVirtualCameraVerified(ready);
    setReadinessVerification("virtualCameraVerified", ready);
  }, []);
  ...
  const handleMeetingPreviewConfirmed = useCallback((ready: boolean) => {
    if (!snapshotLoadedRef.current) return;
    setMeetingPreviewConfirmed(ready);
  ...
  const speechReady =
    ...
    ((speechTestRequestedAt > 0 && diagnostics.lastSpeechAt >= speechTestRequestedAt) ||
      getSnapshotReadiness(loadReadinessSnapshot()).speechReady);

  useEffect(() => {
    const snapshot = getSnapshotReadiness(loadReadinessSnapshot());
    setObsConnected(snapshot.obsConnected);
    setVirtualCameraActive(snapshot.virtualCameraActive);
    setVirtualCameraVerified(snapshot.virtualCameraVerified);
    setVirtualAudioReady(snapshot.virtualAudioReady);
    setMeetingPreviewConfirmed(snapshot.meetingPreviewConfirmed);
  ...
  useEffect(() => {
    if (diagnosticsLoaded) setReadinessVerification("speechReady", speechReady);
  }, [diagnosticsLoaded, speechReady]);
  ...
  async function playTestSpeech() {
    setTestingSpeech(true); setSpeechTestRequestedAt(0); setError("");
    try {
    ...
    } finally { setTestingSpeech(false); }
  }
  ...
  return (
    <main className="console settingsPage">
      ...
      <section className={`readinessBanner ${readiness.ready ? "ready" : ""}`}>
        <div><strong>{readiness.ready ? "面试环境已就绪" : `还有 ${readiness.missing.length} 项需要处理`}</strong><span>{readiness.ready ? "可以返回工作台开始面试。" : readiness.missing.slice(0, 3).map((item) => item.label).join("、")}</span></div>
        <a className="buttonLink" href="/">返回面试工作台</a>
      </section>
      <section className="settingsGrid">
        <div className="settingsSection"><div className="sectionIntro"><span>01</span><div><h2>AI 模型</h2><p>配置用于生成问题与纪要的 OpenAI-compatible 服务。</p></div></div><ModelSettings /></div>
        <div className="settingsSection"><div className="sectionIntro"><span>02</span><div><h2>数字人形象</h2><p>选择会议中显示的面试官图片或待机视频。</p></div></div>
          <article className="card mediaSetup"><div className="cardHeading"><h2>当前画面</h2><span>{avatar.available ? "已启用" : "默认头像"}</span></div><div className="mediaPreview">{avatar.available && avatar.kind === "video" && <video src={`/api/avatar/media?v=${avatar.version}`} autoPlay loop muted playsInline />}{avatar.available && avatar.kind === "image" && <img src={`/api/avatar/media?v=${avatar.version}`} alt="当前数字人素材" />}{!avatar.available && <span>当前使用默认头像</span>}</div><form onSubmit={uploadAvatar}><input name="avatar" type="file" accept="image/jpeg,image/png,image/webp,video/mp4,video/webm" /><button disabled={uploading}>{uploading ? "正在上传…" : "上传并应用"}</button></form>{avatar.available && <><p className="fileMeta">{avatar.originalName} · {Math.max(1, Math.round((avatar.size || 0) / 1024))}KB</p><button className="secondary" disabled={uploading} onClick={clearAvatar}>恢复默认头像</button></>}</article>
        </div>
        <div className="settingsSection"><div className="sectionIntro"><span>03</span><div><h2>OBS 与舞台</h2><p>连接 OBS、配置舞台源并启动虚拟摄像头。</p></div></div><ObsControl onStatusChange={handleObsStatus} /></div>
        <div className="settingsSection"><div className="sectionIntro"><span>04</span><div><h2>摄像头与音频</h2><p>检查会议软件最终会读取到的画面和声音。</p></div></div><div className="settingsPair"><VirtualCameraPreview active={virtualCameraActive} onVerifiedChange={handleCameraVerified} /><AudioRouteControl onReadyChange={handleVirtualAudioReady} /></div></div>
        <div className="settingsSection"><div className="sectionIntro"><span>05</span><div><h2>会议软件确认</h2><p>在入会预览中完成最后一跳确认。</p></div></div><MeetingHandoffControl prerequisitesReady={obsConnected && virtualCameraActive && virtualCameraVerified && virtualAudioReady} onConfirmedChange={handleMeetingPreviewConfirmed} /></div>
        <div className="settingsSection"><div className="sectionIntro"><span>06</span><div><h2>系统诊断</h2><p>状态自动刷新；测试语音完整播放后才会通过语音门禁。</p></div></div><article className="card diagnostics"><div className="checks"><Check label="本地服务" ok={diagnostics.server} detail={diagnostics.server ? "正常" : "不可用"} /><Check label="AI 模型" ok={diagnostics.modelConfigured} detail={diagnostics.modelConfigured ? "已配置" : "尚未配置"} /><Check label="数字人舞台" ok={diagnostics.stageConnected} detail={diagnostics.stageConnected ? "页面在线" : "请在 OBS 或浏览器打开 /stage"} /><Check label="中文语音" ok={speechReady} detail={diagnostics.ttsState === "error" ? describeTtsError(diagnostics.ttsError) : speechReady ? "最近实测播放成功" : diagnostics.sapiConfigured ? `SAPI ${diagnostics.sapiVoiceCount} 个中文声音，等待实测` : "当前舞台不支持 TTS"} /><Check label="画面素材" ok={diagnostics.stageConnected && diagnostics.mediaReady} detail={diagnostics.mediaReady ? "已就绪" : "仍在加载或格式不兼容"} /><Check label="语音转写" ok={diagnostics.transcriptionReady} detail={diagnostics.transcriptionReady ? `${diagnostics.transcriptionProvider} 已就绪` : diagnostics.transcriptionConfigured ? `${diagnostics.transcriptionProvider} 未启动或不可达` : "尚未配置转写服务"} /></div><button disabled={!diagnostics.stageConnected || testingSpeech} onClick={() => void playTestSpeech()}>{testingSpeech ? "正在发送…" : "播放测试语音"}</button></article></div>
      </section>
    </main>
  );
}
...
function Check({ label, ok, detail }: { label: string; ok: boolean; detail: string }) {
  return <div className="check"><i className={ok ? "ok" : ""}>{ok ? "✓" : "!"}</i><div><strong>{label}</strong><span>{detail}</span></div></div>;
}
