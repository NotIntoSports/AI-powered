...
export const READINESS_SNAPSHOT_KEY = "ai-interviewer:readiness:v1";
export const READINESS_VERIFICATION_TTL_MS = 5 * 60_000;

export type ReadinessVerificationId =
  | "speechReady"
  | "obsConnected"
  | "virtualCameraActive"
...
const verificationIds: ReadinessVerificationId[] = [
  "speechReady",
  "obsConnected",
  "virtualCameraActive",
  "virtualCameraVerified",
...
export function getSnapshotReadiness(
  ...
  now = Date.now()
): Pick<InterviewReadinessInput, ReadinessVerificationId> {
  return Object.fromEntries(
    verificationIds.map((id) => [
      id,
      typeof snapshot[id] === "number" && now - snapshot[id]! < READINESS_VERIFICATION_TTL_MS
    ])
  ) as Pick<InterviewReadinessInput, ReadinessVerificationId>;
}

export function updateReadinessSnapshot(
  snapshot: ReadinessSnapshot,
  id: ReadinessVerificationId,
  ready: boolean,
  now = Date.now()
): ReadinessSnapshot {
  const next = { ...snapshot };
  if (ready) next[id] = now;
  else delete next[id];

  if (!next.virtualCameraActive) {
    delete next.virtualCameraVerified;
    delete next.virtualAudioReady;
    delete next.meetingPreviewConfirmed;
  } else if (!next.virtualCameraVerified || !next.virtualAudioReady) {
    delete next.meetingPreviewConfirmed;
  }
  return next;
}
...
export function saveReadinessSnapshot(snapshot: ReadinessSnapshot) {
  if (typeof window === "undefined") return;
  window.sessionStorage.setItem(READINESS_SNAPSHOT_KEY, JSON.stringify(snapshot));
}
...
export function invalidateDeviceReadiness() {
  ...
  for (const id of ["virtualCameraVerified", "virtualAudioReady", "meetingPreviewConfirmed"] as const) {
    snapshot = updateReadinessSnapshot(snapshot, id, false);
  }
