...
const { READINESS_VERIFICATION_TTL_MS, getSnapshotReadiness, parseReadinessSnapshot, updateReadinessSnapshot } = await import("../features/readiness/readiness-snapshot.ts");
const now = 1_000_000;
assert.deepEqual(parseReadinessSnapshot(null), {});
assert.deepEqual(parseReadinessSnapshot("not-json"), {});
assert.deepEqual(parseReadinessSnapshot(JSON.stringify({ speechReady: now, secret: "ignored" })), { speechReady: now });
let snapshot = {};
for (const id of ["speechReady", "obsConnected", "virtualCameraActive", "virtualCameraVerified", "virtualAudioReady", "meetingPreviewConfirmed"]) snapshot = updateReadinessSnapshot(snapshot, id, true, now);
